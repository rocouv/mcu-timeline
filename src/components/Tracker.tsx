...
export default function Tracker() {
  ...
  return <main className="tracker-shell">
    ...
    <section className="progress-section" aria-labelledby="progress-title"><div className="section-heading"><div><span className="eyebrow">ESTADO DE LA MISIÓN</span><h2 id="progress-title">Tu línea temporal</h2></div><button className="reset-button" onClick={reset}>Restablecer progreso</button></div><div className="progress-overview"><div className="progress-ring" style={{ '--progress': `${progress.percentage * 3.6}deg` } as React.CSSProperties}><div><strong>{progress.percentage}<small>%</small></strong><span>completado</span></div></div><div className="progress-copy"><div className="progress-bar"><span style={{ width: `${progress.percentage}%` }} /></div><p><strong>{progress.watched}</strong> vistos <i>/</i> {progress.pending} pendientes <span className="muted">· {formatHours(progress.remainingRuntime)} restantes</span></p><div className="pace-grid"><div><b>{formatHours(progress.weeklyMinutes)}</b><span>por semana</span></div><div><b>{formatHours(progress.dailyMinutes)}</b><span>por día</span></div><div><b>{progress.pendingMovies}</b><span>películas</span></div><div><b>{progress.pendingEpisodes}</b><span>episodios aprox.</span></div></div></div></div></section>
